...
cp python/progress_outbox.py "$DIST_DIR/progress_outbox.py"
...
cp bin/watch-close-progress "$STAGING/bin/"
...
chmod 0755 "$STAGING/bin/sync-native-progress" "$STAGING/bin/watch-close-progress"
cp main.lua "$STAGING/"
...
test -f "$STAGING/dist/progress_outbox.py"
...
test -x "$STAGING/bin/watch-close-progress"
