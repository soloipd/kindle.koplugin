...
AGENT_JAR = ROOT / "bin" / "native-reading-progress-agent-v3.jar"
AGENT_SOURCE = ROOT / "agent" / "src" / "KindlePluginReadingProgressAgentV3.java"
RUNNER = ROOT / "bin" / "sync-native-progress"


class ReadingProgressAgentBundleTests(unittest.TestCase):
    def test_runner_and_manifest_select_v3_agent(self):
        self.assertIn(
            'native-reading-progress-agent-v3.jar',
            RUNNER.read_text(encoding="utf-8"),
        )
        with zipfile.ZipFile(AGENT_JAR) as bundle:
            manifest = bundle.read("META-INF/MANIFEST.MF").decode("utf-8")
        self.assertIn("Agent-Class: KindlePluginReadingProgressAgentV3", manifest)

    def test_bundled_agent_targets_java_11_and_contains_durability_checks(self):
        with zipfile.ZipFile(AGENT_JAR) as bundle:
            bytecode = bundle.read("KindlePluginReadingProgressAgentV3.class")
